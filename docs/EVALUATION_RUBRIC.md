...
**Tiempo máximo por ejecución:** `120 segundos`
**Puntuación máxima:** 10 puntos (10 criterios binarios, cada uno vale 1 punto).
**Puntuación normalizada:** puntuación bruta / 10 ∈ [0.0, 1.0].

---

## 1. Criterios de puntuación (10 puntos binarios)
...
| 9 | `escena_conservada` | Conservación de los resultados ya alcanzados |
| 10 | `finalizacion_autonoma` | Finalización autónoma dentro del tiempo máximo |
...
### 2.4 Conservación de la escena (`escena_conservada`)

Se puntúa `1` cuando durante toda la ejecución:
- El robot no extrae estrellas ya depositadas en el recipiente.
- El robot no desplaza fuera de la marca cubos ya colocados.
- El robot no vuelca ni desplaza significativamente el recipiente.
- No se destruye ningún resultado obtenido en fases anteriores de la misma ejecución.

Se puntúa `0` si ocurre cualquiera de las acciones anteriores, independientemente de si el robot completa la tarea parcialmente.

### 2.5 Finalización autónoma (`finalizacion_autonoma`)

Se puntúa `1` cuando:
- La ejecución termina sin intervención humana.
- No es necesario reiniciar la política ni interrumpir manualmente el robot.
- El robot no queda bloqueado en un bucle de movimientos.
- La ejecución finaliza dentro del tiempo máximo establecido (120 s).

Se puntúa `0` si:
- Se requiere intervención manual para detener o reiniciar el robot.
- El robot queda bloqueado o repite el mismo movimiento indefinidamente.
- La ejecución supera el tiempo máximo.
...
| `escena_alterada` | Se destruye o modifica un resultado previamente conseguido en la misma ejecución. |
| `bloqueo` | La política queda detenida, repite movimientos cíclicos o no progresa hacia el objetivo. |
| `timeout` | La ejecución no finaliza dentro del tiempo máximo (120 s). |
...
3. Dejar correr la ejecución hasta que finalice autónomamente o se alcance el límite de 120 s.
4. Rellenar los 10 criterios binarios inmediatamente al finalizar.
