...
de los 10 criterios binarios del CSV de evaluacion. Muestra resumen por modelo
...
CSV_PATH = Path(__file__).parent / "eval_v2_checklist.csv"
...
def compute_scores(rows: list[dict]) -> tuple[list[dict], list[str]]:
    ...
    for i, row in enumerate(rows):
        ...
        if len(filled) == 10:
            ...
            row["puntuacion_normalizada"] = f"{bruta / 10:.2f}"
            row["exito_completo"]         = "1" if bruta == 10 else "0"
        else:
...
def print_summary(rows: list[dict]) -> None:
    ...
    print(f"\n{sep}")
    print("  RESUMEN POR TIPO DE CONFIGURACION (todos los modelos)")
    print(sep)
    for tipo in ("training", "no_visto", "stress"):
        subset = [r for r in rows if r.get("tipo_configuracion") == tipo]
        if subset:
            print(f"\n  tipo = {tipo}")
            _print_stats("Todos los modelos", _collect_stats(subset))
