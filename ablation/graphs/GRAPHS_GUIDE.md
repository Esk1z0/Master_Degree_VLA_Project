...
### 01 — `01_ablacion_capa_unica.png`
Un único gráfico con **dos barras por capa** (Base + 0..15): la columna izquierda es la puntuación
normalizada media (±desviación estándar) y la columna derecha (rayada) es la tasa de éxito completo, para
poder leer ambas métricas capa a capa sin saltar entre dos figuras. Dos líneas de referencia horizontales
marcan el valor de `base` para cada métrica (discontinua = puntuación, punteada = éxito). Colorea en
**verde** las capas 6/7/8 (elegidas para la inyección de profundidad) y en **rojo** las capas "críticas"
(puntuación media < 0.28 al desactivarlas solas: capas 1, 2, 3, 5, 13, 14); el resto queda en gris. La
clasificación de color se basa siempre en la puntuación, nunca en la tasa de éxito, para que el color de
una capa no cambie según qué barra se esté mirando.

> Se conservan también las dos figuras originales por separado, por si en algún momento son más útiles
> que la versión combinada: [`01a_ablacion_capa_unica_puntuacion.png`](01a_ablacion_capa_unica_puntuacion.png)
> (solo puntuación) y [`01b_ablacion_capa_unica_exito.png`](01b_ablacion_capa_unica_exito.png) (solo tasa
> de éxito). Contenido idéntico al de 01, repartido en dos gráficos en vez de uno.

**Lectura**: las capas 1, 2, 3, 5, 13 y 14 son las más frágiles (puntuación ≤0.26, 0% de éxito completo) —
tocarlas destruye la política. Las capas 6, 7 y 8 están entre las que mejor toleran ser desactivadas
individualmente (puntuación 0.53–0.61, tasa de éxito 14–20%, muy por encima de la mayoría de las demás
capas intermedias/tardías).

### 02 — `02_individual_vs_combinado.png`
...
### 03 — `03_degradacion_no_lineal.png`
...
### 04 — `04_curva_degradacion_capas.png`
...
### 05 — `05_heatmap_config_capas_clave.png`
...
### 06 — `06_modos_fallo_capas_clave.png`
Modo de fallo dominante (`modo_fallo`) entre las ejecuciones fallidas, para los mismos modelos clave que en
05. En `base`/capas individuales el fallo se reparte entre `bloqueo`, `timeout`, `sin_aproximacion`, etc.
